...
function loadManualOverrides() {
  chrome.storage.local.get(['keyOverrides'], (result) => {
    ...
    songKeys.forEach(songKey => {
      ...
      // Escape HTML to prevent XSS from malicious metadata scraped from the DOM
      const escapedTitle = escapeHtml(songTitle);
      const escapedArtist = escapeHtml(songArtist);
      const escapedBadge = escapeHtml(badgeText);
      const escapedSongKey = escapeHtml(songKey);
      
      itemEl.innerHTML = `
        <div class="override-info">
          <span class="override-song">${escapedTitle}</span>
          <span class="override-artist">${escapedArtist}</span>
        </div>
        <div class="override-values-delete">
          <span class="override-badge">${escapedBadge}</span>
          <button type="button" class="override-delete" data-key="${escapedSongKey}" title="Delete override">
